      ...
      try {
        ...
        const domainError =
          error instanceof DomainError
            ? error
            : new DomainError("INTERNAL", "context retrieval failed");
  ...
  server.registerTool(
    ...
    async (params) => {
      try {
        ...
        return okResult(
          `Text inserted (${result.degraded ? "degraded" : "normal"})`,
          toMutationPayload(result),
        );
      } catch (error) {
        const domainError =
          error instanceof DomainError ? error : new DomainError("INTERNAL", "insert failed");
        return errorResult(domainError);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async (params) => {
      try {
        ...
        return okResult(
          `Range replaced (${result.degraded ? "degraded" : "normal"})`,
          toMutationPayload(result),
        );
      } catch (error) {
        const domainError =
          error instanceof DomainError ? error : new DomainError("INTERNAL", "replace failed");
        return errorResult(domainError);
      }
    },
  );
