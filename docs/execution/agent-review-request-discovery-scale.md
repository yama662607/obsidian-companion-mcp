...
- `search_notes_semantic` が全文ではなく候補確認用 excerpt を返すことを確認する
- `get_note` が詳細取得の read path として使えることを確認する
- `move_note` と `get_index_status` が実運用で使えることを確認する
...
3. move_note / get_index_status が運用に足るか確認する
...
5. search で見つかった 1 件に対して get_note を実行する
確認項目:
- ここで初めて全文が取得できる
- discovery -> read の流れが自然

6. get_index_status を実行する
確認項目:
- pendingCount
- indexedCount
- ready
